...
void draw_postfit_inclusive(bool useMT = false) {
  // -----------------------------
  // Input files
  // -----------------------------
  const std::string fitFile = "fitDiagnostics_w_inclusivefit.root";
  const std::string dataFile = "/afs/cern.ch/user/z/zheng/pO_analysis/plotting/plots/combine_input_inclusive.root";
  ...
  if (!fFit || fFit->IsZombie()) {
    std::cerr << "[ERROR] Cannot open " << fitFile << "\n";
    return;
  }
  ...
  if (!hData) {
    std::cerr << "[ERROR] Cannot find data_obs in " << dataFile << "\n";
    return;
  }
  ...
  // -----------------------------
  // Postfit shapes from fitDiagnostics
  // -----------------------------
  // Using signal+background fit
  TH1* hSignal_raw = dynamic_cast<TH1*>(fFit->Get("shapes_fit_s/ch1/signal"));
  TH1* hZ_raw = dynamic_cast<TH1*>(fFit->Get("shapes_fit_s/ch1/z"));
  ...
  TH1* hPdfBkg_raw = dynamic_cast<TH1*>(fFit->Get("shapes_fit_s/ch1/pdfbkg"));
  TH1* hTotal_raw = dynamic_cast<TH1*>(fFit->Get("shapes_fit_s/ch1/total"));

  if (!hSignal_raw || !hZ_raw || !hZtau_raw || !hWtau_raw || !hPdfBkg_raw || !hTotal_raw) {
    std::cerr << "[ERROR] Missing one or more postfit shapes in fitDiagnostics.root\n";
    return;
  }

  TH1* hSignal = RemapToReferenceBinning(hSignal_raw, hData, "signal_postfit");
  TH1* hZ = RemapToReferenceBinning(hZ_raw, hData, "z_postfit");
  ...
  TH1* hPdfBkg = RemapToReferenceBinning(hPdfBkg_raw, hData, "pdfbkg_postfit");
  TH1* hTotal = RemapToReferenceBinning(hTotal_raw, hData, "total_postfit");

  if (!hSignal || !hZ || !hZtau || !hWtau || !hPdfBkg || !hTotal) {
    std::cerr << "[ERROR] Missing one or more postfit shapes in fitDiagnostics.root\n";
    return;
  }

  hSignal = dynamic_cast<TH1*>(hSignal->Clone("signal_postfit"));
  hZ = dynamic_cast<TH1*>(hZ->Clone("z_postfit"));
  hZtau = dynamic_cast<TH1*>(hZtau->Clone("ztau_postfit"));
  hWtau = dynamic_cast<TH1*>(hWtau->Clone("wtau_postfit"));
  hPdfBkg = dynamic_cast<TH1*>(hPdfBkg->Clone("pdfbkg_postfit"));
  hTotal = dynamic_cast<TH1*>(hTotal->Clone("total_postfit"));

  hSignal->SetDirectory(nullptr);
  hZ->SetDirectory(nullptr);
  hZtau->SetDirectory(nullptr);
  hWtau->SetDirectory(nullptr);
  hPdfBkg->SetDirectory(nullptr);
  hTotal->SetDirectory(nullptr);

  RooFitResult* fit_s = dynamic_cast<RooFitResult*>(fFit->Get("fit_s"));
  if (!fit_s) {
    std::cerr << "[ERROR] Cannot find fit_s in fitDiagnostics.root\n";
    return;
  }

  double er = 0, ez = 0, eztau = 0, ewtau = 0, eqcd = 0;
  double r = GetFitPar(fit_s, "r", er);
  double z_rate = GetFitPar(fit_s, "z_rate", ez);
  double ztau_rate = GetFitPar(fit_s, "ztau_rate", eztau);
  double wtau_rate = GetFitPar(fit_s, "wtau_rate", ewtau);
  double pdfbkg_rate = GetFitPar(fit_s, "pdfbkg_rate", eqcd);
  ...
  std::vector<TH1*> bkgs = {hSignal, hZ, hZtau, hWtau, hPdfBkg};

  std::vector<std::string> names = {"Signal", "DY", "DY #tau", "W #tau", "QCD-like"};

  std::vector<std::string> box = {Form("Data: %.0f", hData->Integral(1, hData->GetNbinsX())),
                                  Form("Postfit total: %.1f", hTotal->Integral(1, hTotal->GetNbinsX())),
                                  Form("r = %.3f #pm %.3f", r, er),
                                  Form("z rate = %.3f #pm %.3f", z_rate, ez),
                                  Form("ztau rate = %.3f #pm %.3f", ztau_rate, eztau),
                                  Form("wtau rate = %.3f #pm %.3f", wtau_rate, ewtau),
                                  Form("QCD-like rate = %.3f #pm %.3f", pdfbkg_rate, eqcd)};

  SaveNicePlot1D_WithBkg_Postfit_Pull(hData,
                                 bkgs,
                                 names,
                                 hTotal,
                                 outDir + "/inclusive_postfit",
                                 useMT ? "m_{T} (GeV)" : "PF MET (GeV)",
                                 useMT ? "Events / 2.5 GeV" : "Events / 2.0 GeV",
                                 "",
                                 "W #rightarrow #mu #nu",
                                 "inclusive postfit",
                                 box,
